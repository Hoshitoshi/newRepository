// VisualPareChallenge.tsx
import React, { useState, useEffect } from 'react';
import * as styles from './VisualPareChallenge.css'; // vanilla-extract スタイルのインポート
...
interface VisualPareChallengeProps {
  onAnswer: (answer: boolean) => void;
  onNext: () => void;
}

const VisualPareChallenge: React.FC<VisualPareChallengeProps> = ({ onAnswer, onNext }) => {
  ...
  const [selectedImage, setSelectedImage] = useState<QuizItem | null>(null);
  const [selectedWord, setSelectedWord] = useState<QuizItem | null>(null);
  const [correctPairs, setCorrectPairs] = useState<number[]>([]);
  const [allCorrect, setAllCorrect] = useState(false);
  const [wrongAnswer, setWrongAnswer] = useState<boolean | null>(false);
  const [rightAnswer, setRightAnswer] = useState<boolean | null>(false);

  const isCorrectPair = (item: QuizItem, word: string) => {
  ...
  const handleImageClick = (item: QuizItem) => {
    if (selectedImage && selectedImage.id === item.id) {
      setSelectedImage(null);
    } else if (!selectedWord) {
      setSelectedImage(item);
    } else if (selectedWord && isCorrectPair(item, selectedWord.word)) {
  ...
  const handleWordClick = (item: QuizItem) => {
  ...
  const getImageBackgroundColor = (item: QuizItem) => {
  ...
  const getWordBackgroundColor = (item: QuizItem) => {
    if (correctPairs.includes(item.id)) {
      return styles.correct;
    ...
    <div className={styles.body}>
      <div className={styles.container}>
        {quizData.map(item => (
          ...
          //   className={`${styles.image} ${getImageBackgroundColor(item)}`}
  ...
  alt={item.word}
  onClick={() => handleImageClick(item)}
  className={`${styles.image} ${getImageBackgroundColor(item)} ${correctPairs.includes(item.id) ? styles.imageDisabled : ''}`}
/>
        ))}
      </div>
      ...
      <div className={styles.container}>
        {quizData.map(item => (
          <button
            ...
            className={`${styles.button} ${getWordBackgroundColor(item)} ${correctPairs.includes(item.id) ? styles.imageDisabled : ''}`}
            // className={`${styles.button} ${getWordBackgroundColor(item)}`}
            // disabled={correctPairs.includes(item.id)}
          >
            {item.word}
          </button>
      ...
      {wrongAnswer && <p className={styles.wrongMessage}>ハズレ！</p>}
      {rightAnswer && <p className={styles.correctMessage}>正解！</p>}
      {allCorrect && (
        <div>
          <p className={styles.allCorrectMessage}>全問正解！！◯</p>
          <button className={styles.nextButton} onClick={onNext}>次へ</button>
        </div>
      )}
    </div>
  );
};
export default VisualPareChallenge;
// import React, { useState, useEffect } from 'react';
// import styles from './VisualPare.module.css'; // CSSモジュールのインポート

// export default function VisualPareChallenge({ onAnswer, onNext }) {
//   const quizData = [
//     { id: 1, image: '/image/apple.png', word: 'apple' },
//     { id: 2, image: '/image/cherry.png', word: 'cherry' },
//     { id: 3, image: '/image/banana.png', word: 'banana' },
