...
export default function VisualSoundQuiz({ onAnswer, onNext }) {
  ...
  const ImageDisplay = ({ imageUrl }) => (
    <img src={imageUrl} alt="Learning Object" style={{ maxWidth: '100%' }} />
  );
  ...
  const AudioButton = ({ audioSrc, label, onSelect, isSelected, number }) => {
    ...
    const iconPath = `/image/sound_icon_${number}.png`;
  ...
  const CharacterSpeechBubble = () => {
    const characterImageUrl = '/image/character.JPG';
  ...
  const handleSubmit = () => {
    const correctAnswer = 'apple'; // 正解を設定
    const correct = selectedAnswer === correctAnswer;
    setIsCorrect(correct);
    onAnswer(correct); // QuizHandlerに結果を伝える
...
//   const handleSubmit = () => {
//     const correctAnswer = 'apple'; // 正解をここで設定
//     const correct = selectedAnswer === correctAnswer;
//     setIsCorrect(correct);
//     onAnswer(correct); // 正解の場合はtrue、不正解の場合はfalseを渡す
...
//     const correct = selectedAnswer === correctAnswer;
//     setIsCorrect(correct);
//     console.log('Submitting answer:', correct); // デバッグ用の出力
//     onAnswer(correct); // 正解の場合はtrue、不正解の場合はfalseを渡す
//   return (
//     <div style={{ maxWidth: '600px', margin: 'auto' }}>
//       {/* 画像の表示 */}
